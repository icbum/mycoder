...
The `_snip_tool_outputs()` function in mycoder's `context.py` is the implementation for this layer.
...
mycoder's `_summarize_old()` implements the core logic: it uses the same LLM for summarization, and if the LLM call fails, it falls back to extracting key information (file path, error message) based on regular expressions.
...
mycoder's `_hard_collapse()` implements this layer.
...
mycoder's `maybe_compress()` uses this mechanism—automatically checking at the beginning of each iteration of `agent.chat()` and after each round of tool execution.
...
Claude Code's digest prompt explicitly lists these reserved items. mycoder does the same.

**How many tokens does the digest itself occupy?**

If the digest is too long, compression is wasted. Claude Code limits the output length of digest calls using the `max_tokens` parameter. mycoder limits the digest call prompt to 15,000 characters.
...
Claude Code uses the same model. The cost of one summary call is approximately equal to the cost of one normal conversation. mycoder works similarly. If you want to save money, you can use a cheaper model specifically for summarizing (e.g., using DeepSeek as the primary model but GPT-4o-mini for summarizing).
...
> This article is the 4th in the [Claude Code Source Code Guide](00-index_EN.md) series. Accompanying implementation: [mycoder](../)