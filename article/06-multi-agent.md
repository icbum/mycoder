...
mycoder 也做了同样的设计：`AgentTool.execute()` 创建子 Agent 时，工具列表过滤掉 `agent`。
...
mycoder 也限制了子 Agent 结果的长度（5000 字符上限），防止撑爆主 Agent 的上下文。
...
mycoder 目前只有一种 Agent 类型（通用型），但你可以在 `Agent.__init__` 里传不同的工具列表来模拟：
...
> 本文是 [Claude Code 源码导读](00-index.md) 系列第 6 篇。配套实现：[mycoder](../)
