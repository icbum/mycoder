...
所以 mycoder 选了一个折中方案：不在流中解析工具，但当 LLM 一次返回多个 tool_calls 时，用 `concurrent.futures.ThreadPoolExecutor` 并行执行。
...
这些 edge case 单独处理都不难，但组合在一起就产生了大量的状态管理代码。这也是为什么 mycoder 选择了简化实现——完整版的复杂度跟收益不成正比（对于一个 1300 行的教学项目来说）。

---

> 本文是 [Claude Code 源码导读](00-index.md) 系列第 5 篇。配套实现：[mycoder](../)
