...
> **注意**：Claude Code 源码中 `HISTORY_SNIP` feature flag 存在且被多处引用，但具体实现模块（`snipCompact.js` 等）在公开/泄露的版本中并未包含，属于内部 A/B 测试。Claude Code 目前公开的截断逻辑是 `BashTool/utils.ts` 的 head-only 截断。上面描述的 head+tail 策略是 mycoder 自己的设计选择，不是从 Claude Code 直接移植的。

mycoder 的 `context.py` 里的 `_snip_tool_outputs()` 是这一层的实现。
...
mycoder 的 `_summarize_old()` 实现了核心逻辑：用同一个 LLM 做摘要，如果 LLM 调用失败就 fallback 到基于正则的关键信息提取（文件路径、错误信息）。
...
mycoder 的 `maybe_compress()` 就是这个机制——在 `agent.chat()` 每次循环迭代开头和每轮工具执行后自动检查。
...
Claude Code 的摘要 prompt 里明确列出了这些保留项。mycoder 也是。

**摘要本身占多少 token？**

如果摘要写太长，压缩就白做了。Claude Code 通过 `max_tokens` 参数限制摘要调用的输出长度。mycoder 把摘要调用的 prompt 限制在 15000 字符以内。
...
Claude Code 用的是同一个模型。一次摘要调用的成本 ≈ 一次正常对话的成本。mycoder 也是这样。如果你想省钱，可以用一个便宜的模型专门做摘要（比如用 DeepSeek 做主力但用 GPT-4o-mini 做摘要）。
...
> 本文是 [Claude Code 源码导读](00-index.md) 系列第 4 篇。配套实现：[mycoder](../)
