...
Therefore, mycoder opted for a compromise: it doesn't parse tools in the stream, but when the LLM returns multiple tool_calls at once, it uses `concurrent.futures.ThreadPoolExecutor` to execute them in parallel.
...
These edge cases are not difficult to handle individually, but combined, they generate a large amount of state management code. This is why mycoder opted for a simplified implementation—the complexity of the full version is an overkill for a 1300-line educational project.

---

> This is the 5th article in the [Claude Code Source Code Guide](00-index_EN.md) series. Accompanying implementation: [mycoder](../)