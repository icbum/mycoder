...
def test_config_from_env():
    os.environ["MYCODER_MODEL"] = "test-model"
    c = Config.from_env()
    assert c.model == "test-model"
    del os.environ["MYCODER_MODEL"]


def test_config_defaults(tmp_path, monkeypatch):
    # Run outside the project directory so a local .env file does not affect defaults.
    monkeypatch.chdir(tmp_path)
    for k in ["MYCODER_MODEL", "MYCODER_MAX_TOKENS"]:
...
def test_session_save_load():
    ...
    # cleanup
    pathlib.Path.home().joinpath(".mycoder/sessions/pytest_test_session.json").unlink()
...
def test_session_name_is_sanitized():
    ...
    path = pathlib.Path.home().joinpath(".mycoder/sessions/Research-Notes.json")
    assert path.exists()
